...
typedef struct
{
	...
	//
	// The plugin allocates the return value, the framework frees it using `free_mem`.
	//
	...
	// memory allocated by the plugin that needs to be deallocated by the caller.
	// This applies to all functions that return a char *, such as
	// get_type(), get_name(), get_last_error(), event_to_string(), etc.
